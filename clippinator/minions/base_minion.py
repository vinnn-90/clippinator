...
def get_model(model: str = "gpt-3.5"):
    return ChatOpenAI(
...
class BasicLLM:
    ...
    def __init__(self, base_prompt: str, model: str = "gpt-3.5") -> None:
        llm = get_model(model)
        self.llm = LLMChain(
            llm=llm,
            prompt=PromptTemplate(
...
class BaseMinionOpenAI:
    def __init__(self, base_prompt, available_tools, model: str = "gpt-3.5") -> None:
